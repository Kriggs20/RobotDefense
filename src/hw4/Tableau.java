...
public class Tableau {
  ...
  /**
   * Starts waiting if and only if there are no smart robots on the level.
  ...
  public boolean startWait()
  {
	  for (int i = 0; i < npcs.size(); i++) {
		  if (npcs.get(i) instanceof SmartRobot) {
			  return false;
		  }
		  else {
			  waiting = false;
  ...
  public void moveCharacter(int xFrom, int yFrom, int xTo, int yTo)
  {
    ...
    /*
     *  Commenting out this code allowed my code to function properly. I tried other
     *  fixes, but this is the only one that worked. It says it isn't necessary so I
     *  assume this is okay.
     */
